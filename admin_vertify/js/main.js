 ...
 function(indexPage, editPage, colormap, util) {
  ...
  function renderPage(renderer) {
    ...
    util.requestJSON(dataURL, function(data) {
      ...
      const img_user = urlParams.get('user');
      const ownervertify = urlParams.get('ownervertify');
      ...
      storageRef.child(img_name+".jpg").getDownloadURL().then(function(url) {
        ...
        console.log(img_name+'_'+img_user+'_'+ownervertify+".png");

        storageRef2.child(img_name+'_'+img_user+'_'+ownervertify+".png").getDownloadURL().then(function(url) {
