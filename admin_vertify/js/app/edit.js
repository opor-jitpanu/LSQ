...
define(['../image/layer',
  ...
  function createSidebar(params, data, annotator) {
    ...
    exportButton2 = document.createElement("input"),
    manualText;
    exportButton.type = "submit";
    exportButton2.type = "submit";
    exportButton.value = "Approve";
    exportButton2.value = "Reject";
    exportButton.className = "edit-sidebar-submit btn btn-success";
    exportButton2.className = "edit-sidebar-submit btn btn-danger";

    exportButton.addEventListener("click", function () {
      console.log("Approve This");
      const queryString = window.location.search;
      const urlParams = new URLSearchParams(queryString);
      const user = urlParams.get('ownervertify');
      ...
      firebase.database().ref('user/'+user+'/vertify/'+image).update({
        status:'approve',
        approveby:username,
        approvetime:time
      }, function(error) {
    ...
    container.appendChild(exportButton2);
    return container;
  }
  ...
  function downloadURI(uri, filename) {
    ...
    var file_name = img_name+"_"+username;
    ...
    var file = dataURItoBlob(uri, 'image/png');
    console.log(file);
  ...
  var uploadTask = storageRef.child('annotation/'+img_name+'/'+file_name+'.png').put(file);
