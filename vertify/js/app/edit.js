...
define(['../image/layer',
  ...
  function downloadURI(uri, filename) {
    ...
    console.log(username, image, user_id);
  ...
  var uploadTask = storageRef.child('annotation/'+image+'/'+image+'_'+user_id+'_'+username+'.png').put(file);
  ...
  uploadTask.on('state_changed', function(snapshot){
    ...
    alert(" -2 Vertify Conplete 2- ")
    window.location.href = "../vertify_add_database.html?image="+image+'&user_id='+user_id;
  ...
  function addDatabase() {
    //ไม่ได้ใช้แล้ว
    var ref = firebase.database().ref("vertify/"+img_name);
    ...
    ref.on("value", function(snapshot) {
      ...
      var name = image.name;
      if (name == '') {
        window.location.href = "../editdata_ver.html?data="+user_id+'&image='+img_name;
      }else{
        window.location.href = "../editdata_ver.html?data="+name+'-'+user_id+'&image='+img_name;
      }
